...
function render(app) {
  ...
  loadPages(app.id); document.title = title; document.querySelector('meta[name="description"]').content = shortDescription; document.querySelector('meta[property="og:image"]').content = app.imageUrl || ''; history.replaceState(null, '', `product.html?slug=${encodeURIComponent(app.slug)}`);
}
